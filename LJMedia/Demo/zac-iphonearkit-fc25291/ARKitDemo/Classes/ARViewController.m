...
- (void)loadView {
    ...
    buttonBackToMap = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    buttonBackToMap = [[UIButton alloc] initWithFrame:CGRectMake(162.0f, 0.0f,100.0f, 50.0f)];
    
    [buttonBackToMap setTitle:@"Back" forState:UIControlStateNormal];
    
    [buttonBackToMap setEnabled:YES];
    [buttonBackToMap setSelected:NO];
    buttonBackToMap.alpha = 1.0;
    
    [buttonBackToMap addTarget:delegate action:@selector(BackButtonTapped) forControlEvents:UIControlEventTouchUpInside];
    
    [ar_overlayView addSubview:buttonBackToMap];
